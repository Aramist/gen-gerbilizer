...
from generative import dataloaders as loaders, models
...
def get_args():
    ...
    feather_dir (path): Location of feather files (Pandas dataframes)
    index_dir (path) Location of indices into dataframes
    """
...
def find_data(args, config):
    """ Given command-line arguments and a config file, locates data directories,
    which may be specified in either.
    """
    if args.feather_dir is None or args.index_dir is None:
        # Locate files within config
        if not ('feather_dir' in config and 'index_dir' in config):
            raise ValueError('Paths to dataframes (feather_dir) and indices' \
                '(index_dir) should be provided as command-line arguments' \
                'or entries in the config file.')
        return config['feather_dir'], config['index_dir']
    else:
        return args.feather_dir, args.index_dir
...
def run():
    ...
    feather_dir, idx_dir = find_data(args, config)
